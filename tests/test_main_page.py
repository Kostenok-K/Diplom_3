...
class TestMainPage:
    ...
    @allure.title("Добавление ингредиента,увеличает счетик ингридента в заказе")
    ...
    def test_login_user_create_order(self, main_page, login_page, register_new_user):
        user_data = register_new_user
        login_page.open(LOGIN_PAGE)
        login_page.login(user_data.get("email"), user_data.get("password"))
        main_page.make_order()
